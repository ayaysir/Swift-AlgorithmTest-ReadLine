 ...
 * "비행기 종류의 최소 개수를 출력한다."
  ...
  (1) 최소 신장 트리
   - 신장 트리(Spanning Tree)란 그래프 상에서 모든 노드가 사이클 없이(순환되지 않도록) 연결된 부분 그래프
   - 신장 트리는 방향이 없는 그래프 (무방향 그래프)
   - 그 중 최소 신장 트리(Minimum Spanning Tree)는
     - 간선의 가중치가 모두 동일한 경우 신장 트리 중 가장 적은 간선의 개수를 가진 트리이다.
     - 간선의 가중치가 모두 동일한 그래프의 최소 신장 트리는 간선의 개수가 (정점의 개수 - 1)인 트리이다.
     - 간선의 가중치가 다른 경우 가장 적은 가중치의 합을 가지는 신장 트리이다.
  (2) BFS
