...
func Q_2470() {
    ...
    while l < r {
        let absValue = abs(numbers[l] + numbers[r])
...
/*
 ...
 - 포인터 2개를 만든 뒤, 양 끝에 배치
   - 두 숫자 합의 절대값이 minValue보다 낮은 경우 -> minValue및 result에 할당하되 포인터를 좁히지 않음.
   - 두개 합이 0보다 크면 오른쪽 (큰 숫자) 위치를 줄여야 함 -> 그래야 더 낮은 숫자가 나올 것이므로
   - 두개 합이 0보다 작으면 왼쪽 (작은 숫자) 위치를 줄여야 함 -> 그래야 더 높은 숫자가 나올 것이므로
